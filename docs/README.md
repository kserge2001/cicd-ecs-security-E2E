...
| 02 | [Deploy to EKS the enterprise way (GitOps with Argo CD)](02-deploy-to-eks.md) | Run the same app on an existing EKS cluster using GitOps: Argo CD reconciles a config repo (no cluster creds in CI), ApplicationSet per env, PR + CODEOWNERS promotion gates, Argo Rollouts canary, and External Secrets. CI only bumps the image tag in Git. |
